...
const RecipeCollection: FC<{ path: string; title: string }> = ({
  path,
  title,
}) => {
  ...
  const [cut, setCut] = useState(Math.floor((window.innerWidth -60) / 260));
  ...
  useEffect(() => {
    const handleResize = () => {
      setCut(Math.floor((window.innerWidth - 60) / 260));
    };
    ...
    <article>
      <div className="article-header">
        ...
        <div className="article-header-see-more">
          <Typography
            sx={{ cursor: "pointer" }}
            onClick={() => navigate("/CategoryList" + path)}
          >
            Voir plus
          </Typography>
          <ArrowForwardIcon />
